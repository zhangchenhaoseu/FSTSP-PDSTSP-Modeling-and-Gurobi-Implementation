...
def getSolution(data, model):  # 定义函数，从Gurobi输出的模型中获得目标函数值、决策变量X和Y、继而分别得到卡车和无人机路由的节点序列
    ...
    for v in model.getVars():
        ...
        if split_arr[0] == 'X' and v.x != 0:
            ...
            pass
            # print(v)
    print("卡车路由变量solution.X:",solution.X)
    print("无人机路由变量solution.Y:",solution.Y)
...
def modelingAndSolve(data):
    ...
    for h in data.N_out_lst:
        for k in data.N_in_lst:
            if h != k:
                ...
                num += 1
                m.addConstr(expr + data.costMatrix[h][k] <= M, f'C15_{num}')
    # 16.无人机交付点j的时间约束
    num = 0
    for j in data.C_UAV_lst:
        for i in data.N_out_lst:
    ...
    if m.status == GRB.OPTIMAL:
        ...
        print(f"卡车行驶时间: {m.ObjVal}")
        solution = getSolution(data,m)
        print(f"卡车行驶路线 solution.X_routes: {solution.X_routes}")
        print(f"无人机飞行路线 solution.Y_routes: {solution.Y_routes}")
...
if __name__ =="__main__":
    # 数据集路径
    data_path = r'C:\Users\张晨皓\Desktop\张晨皓的汇报内容\54.问题场景挖掘及综述（二）：FSTSP&PDSTSP建模及其求解\程序代码\data\C101network.txt'  # 这里是节点文件
    customerNum = 10  # C的数量
    UAV_endurance = 10  # 无人机续航
    M = 99999  # 约束中存在的M
    SR = 1  # UAV回收时间
    SL = 1  # UAV发射时间
